...
describe 'Route Matchers' do
  ...
  describe 'assert_routed_to' do
    class MinitestTest
      include Minitest::Assertions
    end
    subject { MinitestTest.new }

    it 'passes when the event is routed only to the specified controller' do
      subject.assert_routed_to create_event('product.update', nil), 'route_spec_product#update_product'
    end

    it 'passes when when the event is routed also to the specified controller' do
      subject.assert_routed_to create_event('product.delete', nil), 'route_spec_product#delete_product'
    end

    it 'fails is not routed to the specified controller' do
      l = lambda { subject.assert_routed_to create_event('product.update', nil), 'route_spec_product#delete_product' }

      l.should raise_error MiniTest::Assertion, "expected event update to be routed to target route_spec_product#delete_product"
    end
